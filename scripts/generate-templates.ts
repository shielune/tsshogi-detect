 ...
 *
 * 以前は隣の mito-context リポジトリのスクリプトが、自分の src/shogi/templates/parser.ts
 ...
 *
 *   bun run scripts/generate-templates.ts             # 両方を書き出す
 *   bun run scripts/generate-templates.ts castles      # 片方だけ
 */

import { readFileSync, writeFileSync } from 'node:fs'
import { join } from 'node:path'
import { PieceType } from 'tsshogi'
import type { ParsedFinishMove, ParsedTemplate, PlacementCell } from '../src/parser.ts'
import { parseTemplateFile } from '../src/parser.ts'
import type { TemplateFinishCapture, TemplateSquare } from '../src/template.ts'
...
const TARGETS: Readonly<Record<string, Target>> = {
  castles: {
    ...
    typeImport: "import type { FormationTemplate } from './template.ts'",
...
const square = (value: TemplateSquare): string => `{ file: ${value.file}, rank: ${value.rank} }`

const squares = (values: readonly TemplateSquare[]): string =>
...
function capture(value: TemplateFinishCapture): string {
...
/** テンプレ 1 件のオブジェクトリテラル。キーの並びは FormationTemplate の宣言順。 */
function template(parsed: ParsedTemplate): string {
...
function header(target: Target, body: string): string {
  ...
  return [
    '// GENERATED by scripts/generate-templates.ts. DO NOT EDIT.',
...
for (const name of names) {
  ...
  const body = parsed.map((entry) => template(entry)).join('\n')
