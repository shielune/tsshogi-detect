...
指し手から手筋と格言パターンを検出する。
定義エンジン（castle.dart / move_history.dart）と手筋の基礎実装は
...
// => [{ definition: { name: '金矢倉', ... }, side: 'black' }]

// 棋譜を走査して「初めて成立した手」を得る
const at = recordCastles(moves)
// => [{ definition, side, ply: 34 }]

// 戦法も同じ形で返る
const strategies = recordStrategies(moves)
// => [{ definition: { name: '四間飛車', ... }, side: 'black', ply: 12 }]
```
...
手筋と格言は定義照合を使わず、指し手と前後の局面を直に見る別系統で、
`src/technique.ts` と `src/proverb.ts` に閉じている。

- `src/definition.ts` — 定義の型 `FormationDefinition`
- `src/requirements.ts` — 定義を構成する要件 `DefinitionRequirement`（盤上セル / 盤面全体 / 履歴依存）
...
- `src/match.ts` — 1 局面 1 定義の照合
- `src/hierarchy.ts` — 系統（`parent`）をたどる親ゲート
- `src/scan.ts` — 棋譜の走査（`recordDefinitions`）
...
- `src/castles.gen.ts` — 囲い定義 113 件（生成物、手で編集しない）
...
定義の正は `data/castles.txt` と `data/strategies.txt`（bioshogi 由来の構造化データ、
...
読むのは同梱の解析器（`parseDefinitionFile`）なので、拡張記法（`finish:` `category:`
`?` による OR 指定、`no_drop:` `bishop_exchange:`）もそのまま通る。
`data/*.txt` を変更したら再生成してコミットする。

## 版の差分

どの定義が増えたか、どの定義が変わったかを版どうしで比べる。
...
- 囲いと戦法: 局面・棋譜から成立形を検出する。同梱する定義は囲い 113 件と戦法 244 件。
