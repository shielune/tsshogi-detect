...
// 同じ手で成立する定義を並べ、決着が付く段を 1 つずつ確かめる。
...
/** 玉が定位置に居れば成立する定義。初形で両陣営とも当たる。 */
const king = (name: string, extra?: Partial<FormationDefinition>): FormationDefinition => ({
  name,
  placements: [new PiecePlacement(5, 9, PieceType.KING)],
  ...extra,
})

/** 玉に 7 七の歩を足した、1 つ狭い定義。これも初形で当たる。 */
const kingAndPawn = (name: string, extra?: Partial<FormationDefinition>): FormationDefinition => ({
  name,
  placements: [new PiecePlacement(5, 9, PieceType.KING), new PiecePlacement(7, 7, PieceType.PAWN)],
  ...extra,
})

/** 6 六の歩。3 手目で初めて成立する、後から立つ側の定義。 */
const pawn66 = (name: string, priority: number): FormationDefinition => ({
  name,
  priority,
...
describe('detectDefinitions', () => {
  ...
  test('優先度が大きいほど前に出る', () => {
    const detected = detectDefinitions([king('一'), king('二', { priority: 10 })], position)
    expect(names(detected)).toEqual(['二/black', '二/white', '一/black', '一/white'])
  })

  test('負の数を書けば後ろへ回る', () => {
    const detected = detectDefinitions([king('一', { priority: -10 }), king('二')], position)
    expect(names(detected)).toEqual(['二/black', '二/white', '一/black', '一/white'])
  })

  test('優先度が同じならネストの深い方が前', () => {
    // 子は親を狭めた形なので、両方成立したら子を代表として扱いたい。
    const definitions = [king('親'), king('子', { parent: '親' }), king('孫', { parent: '子' })]
    expect(names(detectDefinitions(definitions, position))).toEqual([
      '孫/black',
      '孫/white',
      '子/black',
      '子/white',
  ...
  test('深さが同じなら制約の厳しい方が前', () => {
    // 系統の付いていない定義どうしは、要件の数で狭さを測る。
    const definitions = [king('緩い'), kingAndPawn('狭い')]
    expect(names(detectDefinitions(definitions, position))).toEqual([
      '狭い/black',
      '狭い/white',
      '緩い/black',
  ...
  test('どれも同じなら名前の文字コード順', () => {
    ...
    expect(names(detectDefinitions([king('二'), king('一')], position))).toEqual([
      '一/black',
      '一/white',
      '二/black',
      '二/white',
  ...
  test('件数は変わらない', () => {
    const definitions = [king('一'), king('二', { priority: 10 }), king('三', { priority: -10 })]
    expect(detectDefinitions(definitions, position)).toHaveLength(6)
  })
})

describe('recordDefinitions', () => {
  const moves = play('7g7f 3c3d 6g6f')

  test('盤の外の条件も厳しさに数える', () => {
    // 手数の制約は局面 1 枚では検証できないので、走査の側で見る。
    // 升の要件は同じ数なので、plyMax を書いた二の方が 1 つ狭い。
    const definitions = [king('一'), king('二', { plyMax: 40 })]
    expect(namesAt(recordDefinitions(definitions, moves))).toEqual([
      '二/black@1',
      '二/white@1',
      '一/black@1',
      '一/white@1',
  ...
  test('並べ替えは同じ手数の固まりの中だけ', () => {
    const definitions = [king('一'), king('二', { priority: 10 }), pawn66('三', 100)]
