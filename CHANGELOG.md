...
- テンプレートの優先度 (`priority:`)。同じ手数で複数の囲いや戦法が同時に成立したとき、
  返す配列の**どこに置くか**を定義の側から言えるようにした。大きいほど前に出る。
  省略は 0 なので、既存の囲い 113 件と戦法 244 件は 1 行も変わっておらず、見え方も
  今までのままになる。後ろへ回したければ負の数も書ける。
  - **落とす数は変えない**。成立したものは今までどおり全部返り、順番だけが変わる。
  - 並べ替えるのは**同じ手数の中だけ**。`recordCastles` / `recordStrategies` では
    手数が今までどおり主たる順序で、優先度はその中の決着の付け方になる。
    手数を持たない `detectCastles` / `detectStrategies` は配列全体が対象。
  - 優先度が同じなら今までの順 (分類の深さ、定義ファイルの並び) がそのまま残る。
  - 成立の可否には関わらないので、盤の形を持たない分類の節 (`category: true`) にも
    書ける。振り飛車と四間飛車が同じ手で成立したときに、どちらを先に出すかを言える。
- 優先度で並べ替える関数 `sortByPriority` / `sortByPriorityWithinPly` と、
  テンプレ 1 件の優先度を読む `priorityOf`。自前のテンプレート列を
