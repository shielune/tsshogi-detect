...
import type { DefinitionRequirement, DefinitionSquare } from './requirements.ts'

/** 升の型は要件の語彙なので requirements.ts が持つ。定義側からも引けるようにする。 */
export type { DefinitionSquare }
 ...
 */
export type DefinitionFinishCapture =
  /** 何かを取る手 (`x *`)。駒種は問わない。 */
  | { readonly kind: 'any' }
  /** 何も取らない手 (`x _`)。 */
  | { readonly kind: 'none' }
 ...
 */
export interface DefinitionFinishMove {
  readonly from?: DefinitionSquare
  ...
  /** 取った駒の指定。省略すると問わない。 */
  readonly capture?: DefinitionFinishCapture
...
/** 囲い・戦法共通の定義 (位置ベースのパターンマッチ)。 */
export interface FormationDefinition {
  readonly name: string
  readonly aliases?: readonly string[]
  /** 親分類 (カニ囲い等)。親自身も定義として存在することがある。 */
   ...
   */
  readonly finishMoves?: readonly DefinitionFinishMove[]
  readonly placements: readonly DefinitionRequirement[]
}
...
/** 棋譜の中で定義が初めて成立した手。 */
export interface DetectedDefinitionAt extends DetectedDefinition {
  readonly ply: number
}
