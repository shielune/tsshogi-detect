...
export type DetectedCastleAt = DetectedTemplateAt
...
export function detectCastles(
  ...
  return detectTemplates(KNOWN_CASTLES, position, side, history)
}
 ...
 * - 居玉のような game-end 評価テンプレは走査後に 1 度だけ評価し、ちゃんとした囲いが
 *   成立している陣営には出さない
 */
export function recordCastles(moves: readonly Move[], initial?: Position): DetectedCastleAt[] {
  return recordTemplates(KNOWN_CASTLES, moves, {
    initial,
