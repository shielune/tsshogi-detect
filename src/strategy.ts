 ...
 * - 居玉のような game-end 評価定義は抑制しない (囲いだけの都合)
 *
...
/**
 * 戦法定義。中身は `FormationDefinition` (囲いと戦法で同じ形) と同じもの。
 */
export type StrategyDefinition = FormationDefinition

export type DetectedStrategy = DetectedDefinition

export type DetectedStrategyAt = DetectedDefinitionAt
...
export function detectStrategies(
  ...
  return detectDefinitions(KNOWN_STRATEGIES, position, side, history)
}
...
export function recordStrategies(
  ...
  return recordDefinitions(KNOWN_STRATEGIES, moves, {
    initial,
    moverOnly: true,
    requireParent: true,
  })
